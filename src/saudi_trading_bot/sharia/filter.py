...
class StrictShariaFilter:
    ...
    def __init__(self, csv_path: str | Path, max_source_check_age_days: int = 14, block_unknown: bool = True):
    ...
    def check(self, symbol: str, today: date | None = None) -> ShariaDecision:
        ...
        if rows.empty:
            return ShariaDecision(str(symbol), not self.block_unknown, "unknown", "", "", None, False, "not in allowlist")
        row = rows.iloc[-1]
        ...
        stale = checked_date is None or (today - checked_date).days > self.max_source_check_age_days
        status = str(row.get("status", "unknown")).lower().strip()
        allowed = status == "allowed" and not stale
        reason = "allowed" if allowed else ("stale source validation" if stale else f"status={status}")
